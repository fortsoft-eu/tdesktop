...
int LocalStorage::Row::resizeGetHeight(int newWidth) {
	...
	const auto available = std::max(newWidth - padding.left() - padding.right()
		- (_clear->isHidden() ? 0 : _clear->width() + padding.right()), 0);
...
LocalStorage::Database::TaggedSummary LocalStorage::summary() const {
...
not_null<Ui::MediaSlider*> LocalStorage::createLimitsSlider(
	...
	const auto slider = container->add(
		object_ptr<Ui::MediaSlider>(container, st::classicLocalStorageLimitSlider),
		st::classicLocalStorageLimitMargin);
	slider->resize(st::classicLocalStorageLimitSlider.seekSize);
